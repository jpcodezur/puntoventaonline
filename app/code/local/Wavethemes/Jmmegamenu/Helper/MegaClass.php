        ...
        foreach ($collections as $item) {
            ...
            if (is_array($blocks) && count($blocks) > 0) {
                ...
                for ($col = 0; $col < $cols; $col++) {
                    ...
                    if ($cols > 1) $item->content .= $this->beginSubMenuModules($item, 1, $pos, $col, true);
                    $i = $col;
                    while ($i < $total) {
                        $block = $blocks[$i];
                        $i += $cols;
...
//	}
    ...
    public function beginSubMenuModules($item, $level = 0, $pos, $i, $return = false)
    {
        ...
        if ($level) {
            ...
            if ($item->megaparam->get('group')) {
                ...
                if (!$colw) $colw = $this->items[$item->parent]->megaparam->colw ? $this->items[$item->parent]->megaparam->colw : 200;
    ...
    public function beginMenu()
    {
        ...
        echo "<div class=\"{$animation} jm-megamenu clearfix\" id=\"jm-megamenu\">\n";
    }
    ...
    public function genClass($mitem, $level, $pos)
    {
        ...
        if (@$this->children[$mitem->menu_id] || (isset($mitem->content) && $mitem->content)) {
            ...
            else if ($level < $this->params->end_level) $cls .= " haschild";
        }
